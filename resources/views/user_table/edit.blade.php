...
@section('heading')NetPing - обновить пользователя <strong>{{ $user->name }}</strong>@endsection

@section('content')
<div class="mt-5 md:m-0 md:m-auto md:w-2/4 justify-center">
    <form method="post" action="{{ route('update_user_info', $user->id) }}">
        @csrf
        <div class="shadow overflow-hidden sm:rounded-md">
            ...
            <div class="px-4 py-5 bg-white sm:p-6 dark:bg-gray-600">
                <label for="user_pass" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Изменить пароль</label>
            ...
            <div class="px-4 py-2 bg-white sm:p-6 border-t dark:border-gray-500 dark:bg-gray-600">
                <input type="checkbox" class="rounded focus:outline-none focus:ring dark:ring-offset-gray-600 dark:focus:ring-gray-500 border-gray-300 dark:border-gray-600 dark:bg-gray-500 dark:text-gray-400" id="order_email" name="user_is_admin" {{ $user->is_admin == 1 ? 'checked':''}}><span class="px-2 dark:text-gray-100">Администратор</span>
            </div>
